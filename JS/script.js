var countries = ["Afghanistan", "Åland Islands", "Albania", "Algeria", "American Samoa",
    ...
    "State Of", "Panama", "Papua New Guinea", "Paraguay", "Peru", "Philippines", "Pitcairn",
...
var params = JSON.stringify(countries);
...
$.ajax({
    ...
    $.getJSON('JS/test.json', function (data) {
        for (var i = 0; i < data.length; i++) {
            var options = $( "<option></option>" ).appendTo( "select" );
            $(options).text(data[i]);
        }
    });

$(".form-control").change(function(){
    document.getElementById("demo").innerText = ($(".form-control :selected").html());
});
